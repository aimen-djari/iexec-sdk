...
const voucher = require('../common/modules/voucher');
...
class IExec {
	constructor(
		...
		this.voucher = {};
		this.voucher.deposit = async (address, amount) => voucher.deposit(await getContracts(), address, amount);
		this.voucher.requestTask = async (
			{
				apporder,
				datasetorder = order.NULL_DATASETORDER,
				workerpoolorder,
		...
		this.voucher.show = async (address) => voucher.show(await getContracts(), address);
		this.voucher.countApps = async () => voucher.countApps(await getContracts());
		this.voucher.countDatasets = async () => voucher.countDatasets(await getContracts());
		this.voucher.countWorkerpools = async () => voucher.countWorkerpools(await getContracts());
		this.voucher.viewApp = async (id) => voucher.viewApp(await getContracts(), id);
		this.voucher.viewDataset = async (id) => voucher.viewDataset(await getContracts(), id);
		this.voucher.viewWorkerpool = async (id) => voucher.viewWorkerpool(await getContracts(), id);
		this.voucher.addApp = async (address) => voucher.addApp(await getContracts(), address);
		this.voucher.addDataset = async (address) => voucher.addDataset(await getContracts(), address);
		this.voucher.addWorkerpool = async (address) => voucher.addWorkerpool(await getContracts(), address);
		this.voucher.removeApp = async (address) => voucher.removeApp(await getContracts(), address);
		this.voucher.removeDataset = async (address) => voucher.removeDataset(await getContracts(), address);
		this.voucher.removeWorkerpool = async (address) => voucher.removeWorkerpool(await getContracts(), address);

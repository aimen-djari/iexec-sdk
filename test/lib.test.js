...
const { signRequestorder } = require('../src/common/modules/order');
...
describe('[voucher]', () => {
	...
	test('voucher.countApps()', async () => {
		...
		let res = await iexec.voucher.countApps();
		expect(res instanceof BN).toBe(true);
		expect(res.eq(new BN('0'))).toBe(true);

		const apporder = await deployAndGetApporder(iexec);
		await iexec.voucher.addApp(apporder.app);

		res = await iexec.voucher.countApps();
		expect(res.eq(new BN('1'))).toBe(true);

		res = await iexec.voucher.viewApp(0);
		expect(res).toBe(apporder.app);

		await iexec.voucher.removeApp(apporder.app);

		res = await iexec.voucher.countApps();
		expect(res.eq(new BN('0'))).toBe(true);
	});

	test('voucher.countDatasets()', async () => {
		const signer = utils.getSignerFromPrivateKey(tokenChainUrl, PRIVATE_KEY);
		const iexec = new IExec(
			{
				ethProvider: signer,
		...
		let res = await iexec.voucher.countDatasets();
		expect(res instanceof BN).toBe(true);
		expect(res.eq(new BN('0'))).toBe(true);

		const datasetorder = await deployAndGetDatasetorder(iexec);
		await iexec.voucher.addDataset(datasetorder.dataset);

		res = await iexec.voucher.countDatasets();
		expect(res.eq(new BN('1'))).toBe(true);

		res = await iexec.voucher.viewDataset(0);
		expect(res).toBe(datasetorder.dataset);

		await iexec.voucher.removeDataset(datasetorder.dataset);
		res = await iexec.voucher.countDatasets();
		expect(res.eq(new BN('0'))).toBe(true);
	});

	test('voucher.countWorkerpools()', async () => {
		const signer = utils.getSignerFromPrivateKey(tokenChainUrl, PRIVATE_KEY);
		const iexec = new IExec(
			{
				ethProvider: signer,
		...
		let res = await iexec.voucher.countWorkerpools();
		expect(res instanceof BN).toBe(true);
		expect(res.eq(new BN('0'))).toBe(true);

		const workerpoolorder = await deployAndGetWorkerpoolorder(iexec);
		await iexec.voucher.addWorkerpool(workerpoolorder.workerpool);

		res = await iexec.voucher.countWorkerpools();
		expect(res.eq(new BN('1'))).toBe(true);

		res = await iexec.voucher.viewWorkerpool(0);
		expect(res).toBe(workerpoolorder.workerpool);

		await iexec.voucher.removeWorkerpool(workerpoolorder.workerpool);
		res = await iexec.voucher.countWorkerpools();
		expect(res.eq(new BN('0'))).toBe(true);

	});

	test('voucher.show()', async () => {
		const signer = utils.getSignerFromPrivateKey(tokenChainUrl, PRIVATE_KEY);
		const iexec = new IExec(
			{
				ethProvider: signer,
		...
		let workerpoolorder = await deployAndGetWorkerpoolorder(iexec);
		let requestorder1 = await getMatchableRequestorder(iexec, {
			apporder,
			datasetorder,
			workerpoolorder,
		});
		const requestorder = await iexec.order.signRequestorder(requestorder1, {
			checkRequest: false,
		});
		...
		const showDealRes = await iexec.deal.show(res.dealid);
		expect(showDealRes).toBeDefined();
		expect(showDealRes.requester).toBe(hubAddress);
		expect(showDealRes.beneficiary).toBe(ADDRESS);
